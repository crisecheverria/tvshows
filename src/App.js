...
async function fetchEpisodes({imdbID}) {
  try {
    const res = await fetch(`https://www.omdbapi.com/?i=${imdbID}&plot=short&r=json&apikey=a7da74f5`);
    const data = res.json();
...
function App() {
  ...
  async function fetchTvShow() {
    try {
      ...
      const averageRating = response.Episodes.reduce((prev, curr) => {
      ...
      const totalRuntime = episodesInformation
        .reduce((prev, curr) => {
          return prev + (+curr.Runtime.replace(/\D/g, ""));
        }, 0);

      const mostUsedWordPerPlot = episodesInformation.map(({Title, Plot}) => mostUsedWord(Plot, Title)).sort((a, b) => (a.num - b.num));
      ...
      for(let i = 0; i < mostUsedWordPerPlot.length; i++) { 
        let words = mostUsedWordPerPlot[i]['words'];
        num = mostUsedWordPerPlot[i]['num'];
        let title = mostUsedWordPerPlot[i]['title'];
        if (words.length > 1) {
          for (let w of words) {
            if (lookup[w] == undefined) {
              lookup[w] = 0;
            }
            lookup[w] += num
            if(lookup[w] > best) {
              ...
              episodeTitle = [title];
              best = lookup[w];
            } else if (lookup[w] === best) {
              mostFrequentWord.push(w);
              episodeTitle.push(title);
              best = lookup[w];
            }
          }
        }
              ...
              <Grid item key={1} xs={12} sm={6} md={8}>
                ...
                <Card className={classes.card}>
                  <CardContent className={classes.cardContent}>
                    ...
                    <Typography>
                      Episode Title with most occurrences of the most frequent word: 
                    </Typography>
                    <Typography className={classes.boldText}>
